...
export default function EditorLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100">
      ...
      <aside className="flex w-14 flex-col items-center gap-4 border-r border-slate-800 bg-slate-950 py-4">
        <Link
          href="/home"
...
function EditorNavIcon({ href, label, children }: EditorNavIconProps) {
  return (
    ...
    >
      {children}
      <span className="pointer-events-none absolute left-full ml-2 inline-flex translate-y-0 rounded-md bg-slate-900 px-2 py-1 text-xs font-medium text-slate-100 opacity-0 shadow-lg ring-1 ring-slate-700 transition group-hover:opacity-100">
        {label}
      </span>
    </Link>
  );
