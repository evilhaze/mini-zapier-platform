  ...
  return (
    <ThemeProvider>
      <div className="flex min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
        ...
        <aside className="relative z-10 flex w-14 flex-shrink-0 flex-col items-center gap-4 overflow-visible border-r border-slate-200/80 bg-white py-3 dark:border-slate-700 dark:bg-slate-900">
          <Link
            href="/home"
        ...
        {/* Editor workspace */}
        <div className="flex min-w-0 flex-1 flex-col bg-slate-50 dark:bg-slate-900">{children}</div>
      </div>
    </ThemeProvider>
  );
}
  ...
  return (
    <Link
      ...
      className={`
        ...
        ${active
          ? 'border-red-500/30 bg-gradient-to-b from-red-500/20 to-slate-50/80 text-slate-900 shadow-[0_10px_24px_-14px_rgba(239,68,68,0.75)] ring-1 ring-red-500/20 dark:from-red-500/25 dark:to-slate-800 dark:text-slate-50 dark:ring-red-500/30'
          : 'border-transparent bg-slate-50 text-slate-500 hover:border-slate-200 hover:bg-white hover:text-slate-900 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-50'
        }
    ...
    >
      ...
      <span className="pointer-events-none absolute left-full top-1/2 z-20 ml-2 inline-flex -translate-y-1/2 whitespace-nowrap rounded-md bg-slate-900 px-2 py-1 text-xs font-medium text-slate-100 opacity-0 shadow-lg ring-1 ring-slate-700 transition group-hover:opacity-100 dark:bg-slate-700 dark:ring-slate-600">
        {label}
      </span>
    </Link>
  );
